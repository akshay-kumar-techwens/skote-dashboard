...
// Month Data (1-30)
const monthData: DataPoint[] = Array.from({ length: 30 }, (_, i) => {
  const day = i + 1;
  const seed = (i * 13) % 100;
  return {
    name: `${day}`,
    ...
    <div className="w-full bg-white rounded-lg shadow-[0_0.75rem_1.5rem_rgba(18,38,63,0.03)] border border-gray-100 p-6 h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex items-center space-x-1 bg-[#f8f9fa] p-1 rounded-md self-end sm:self-auto">
          {(['Week', 'Month', 'Year'] as FilterType[]).map((filter) => (
            <button
              key={filter}
              onClick={() => setActiveFilter(filter)}
              className={`px-3 py-1 text-[13px] font-medium transition-all rounded-[4px] ${
                activeFilter === filter
